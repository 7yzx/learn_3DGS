...
#TODO: clean it
#这些函数用于读取图像的 EXIF 数据，并将 GPS 坐标转换为十进制度数。
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_seq_matches_per_view', default=0, type=int)  # 每个视图的序列匹配数量
    parser.add_argument('--n_quad_matches_per_view', default=10, type=int)  # 每个视图的四元匹配数量。
    parser.add_argument('--n_loop_closure_match_per_view', default=5, type=int)  # 每个视图的循环闭合匹配数量。
    parser.add_argument('--loop_matches', default=[], type=int)   # 循环匹配的数组。
    parser.add_argument('--n_gps_neighbours', default=25, type=int)  # 每个图像的 GPS 邻居数量。
    ...
    for cam_id, current_cam in enumerate(image_files_organised):  # 遍历image file，
        for matched_cam_id, matched_cam in enumerate(image_files_organised[cam_id:]):  # 每一次都是去找后面的没有匹配过的
            for current_image_id, current_image_file in enumerate(current_cam['images']):
                for frame_step in range(args.n_seq_matches_per_view):  # 序列匹配策略
                    matched_frame_id = current_image_id + frame_step
                    add_match(cam_id, matched_cam_id, current_image_file, matched_frame_id)

                for match_id in range(args.n_quad_matches_per_view):  # 四元组匹配策略
